...
def vehiculos_delete(vehiculo_id):
    if not session.get('es_administrador'):
        abort(403)

    try:
        result = call_stored_procedure('sp_AdminEliminarVehiculo', [vehiculo_id])

        # Si el SP devuelve un SELECT con mensaje
        if isinstance(result, list) and result:
            mensaje = result[0].get('mensaje', 'Vehículo eliminado correctamente.')
            flash(mensaje, 'success')
        else:
            flash('Vehículo eliminado correctamente.', 'success')

    except Exception as e:
        # Mensaje más entendible para el usuario
        texto = str(e)
        if '60001' in texto:
            flash('No se puede eliminar el vehículo porque tiene ventas activas o finalizadas.', 'danger')
        else:
            flash(f'No se pudo eliminar el vehículo: {texto}', 'danger')
...
def vehiculos_nuevo():
    if request.method == 'POST':
        ...
        if not all([marca, modelo, anio, precio, color, tipo]):
            flash('Marca, modelo, año, precio, color y tipo son obligatorios.', 'danger')
            return render_template(
                'admin/vehiculos/form.html',
                modo='nuevo',
        ...
        execute_query("""
            ...
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
        """, (marca, modelo, anio, precio, color, tipo,
              estado, descripcion, imagen_url), fetch=False)
...
def vehiculos_editar(vehiculo_id):
    if request.method == 'POST':
        ...
        if not all([marca, modelo, anio, precio, color, tipo]):
            flash('Marca, modelo, año, precio, color y tipo son obligatorios.', 'danger')
            return redirect(url_for('admin.vehiculos_editar', vehiculo_id=vehiculo_id))
        ...
        execute_query("""
            ...
            WHERE vehiculo_id = ?
        """, (marca, modelo, anio, precio, color, tipo,
              estado, descripcion, imagen_url, vehiculo_id), fetch=False)
    ...
    return render_template('admin/vehiculos/form.html', modo='editar', vehiculo=rows[0])
...
def clientes_nuevo():
    if request.method == 'POST':
        ...
        if not all([nombre, email, password]):
            ...
            return render_template('admin/clientes/form.html', modo='nuevo', cliente=request.form)
        ...
        """, (nombre, email, telefono, direccion,
              tipo_doc, num_doc, password_hash), fetch=False)
    ...
    if request.method == 'POST':
        ...
        if not all([nombre, email, puesto, password]):
            ...
            return render_template(
                ...
                modo='nuevo',
                empleado=request.form
            )

        # Hash de contraseña
        password_hash = User.hash_password(password)

        # Inserción (nota: puesto debe ser uno de: Gerente, Vendedor, Administrativo)
        execute_query("""
            INSERT INTO Empleados
                (nombre_completo, email, puesto, password_hash,
                 es_administrador, activo)
            VALUES (?, ?, ?, ?, ?, 1)
        """, (nombre, email, puesto, password_hash, es_admin), fetch=False)
...
def empleados_editar(empleado_id):
    if request.method == 'POST':
        ...
        if not all([nombre, email, puesto]):
            ...
            return redirect(url_for('admin.empleados_editar', empleado_id=empleado_id))
        ...
        execute_query("""
            ...
            SET nombre_completo = ?, email = ?, puesto = ?,
                es_administrador = ?, activo = ?
            WHERE empleado_id = ?
        """, (nombre, email, puesto, es_admin, activo, empleado_id), fetch=False)

        # Si envía nueva contraseña, actualizar hash
        if password:
            password_hash = User.hash_password(password)
            execute_query("""
                UPDATE Empleados
                SET password_hash = ?
                WHERE empleado_id = ?
            """, (password_hash, empleado_id), fetch=False)
    ...
    return render_template('admin/empleados/form.html', modo='editar', empleado=rows[0])
...
def clientes_editar(cliente_id):
    if request.method == 'POST':
        ...
        execute_query("""
            ...
            WHERE cliente_id = ?
        """, (nombre, email, telefono, direccion,
              tipo_doc, num_doc, activo, cliente_id), fetch=False)
        ...
        if password:
            ...
            execute_query("""
                ...
                SET password_hash = ?
                WHERE cliente_id = ?
            """, (password_hash, cliente_id), fetch=False)
    ...
    return render_template('admin/clientes/form.html', modo='editar', cliente=rows[0])
